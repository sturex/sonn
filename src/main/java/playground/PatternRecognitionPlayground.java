...
public class PatternRecognitionPlayground {
    ...
    public static final int patternLength = 50;
    public static int[] inputArray;

    public static void main(String[] args) throws InterruptedException {

        PatternGenerator patternGenerator = PatternGenerator.newBuilder(patternSize)
                .addPattern(Pattern.randomized(patternSize, patternLength, 10), 1, 0)
                .withLimit(1000)
                .withProbability(1)
                .build();
...
//            network.addAdaptiveDictReceptor(() -> inputArray[channelIdx], Set.of(0));
        ...
        for (int[] ints : patternGenerator) {
            inputArray = ints;
            System.out.println(idx++ + ": " + Arrays.toString(inputArray));
            network.tick();
            Thread.sleep(100);
        }
    }
